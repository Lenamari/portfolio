...
// BrowserSync http://localhost:3000/ Reload without refresh
...
// Clean dist Clean folders before starting again the dist folder
gulp.task('clean', () => {
...
gulp.task('style', () => {
  gulp
    ...
    .pipe(plugin.csscomb())
    .pipe(config.isProd ? plugin.cssnano() : plugin.util.noop()) //est-ce que c'est en prod ? si oui, CSS nano (minifie) sinon rien (noop = rien)
